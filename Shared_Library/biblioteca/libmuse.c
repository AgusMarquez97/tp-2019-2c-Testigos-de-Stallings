...
int muse_init(int id, char* ip, int puerto) {
	...
	if(socketCliente != -1) {
		...
		loggearWarning("No se ha podido inicializar la biblioteca MUSE");
	}
	return retorno;
...
uint32_t muse_alloc(uint32_t tam) {
	...
	if(socketCliente != -1) {
		...
		if(direccionMemoria != 0) {
			...
			loggearWarning("No se ha podido reservar la porción de memoria solicitada");
		}
	}
...
void muse_free(uint32_t dir) {
	...
	if(socketCliente != -1) {
		...
		if(retorno != 0) {
			...
			loggearWarning("No se ha podido liberar la porción de memoria solicitada");
		}
	}
...
uint32_t muse_map(char* path, size_t length, int flags) {
	...
	if(socketCliente != -1) {
		...
		if(direccionMemoria != 0) {
				...
				loggearWarning("No se ha podido mappear la información");
			}
	}
...
int muse_sync(uint32_t addr, size_t len) {
	...
	if(socketCliente != -1) {
		...
		if(retorno != -1) {
				...
				loggearWarning("No se ha podido sincronizar la información");
			}
	}
...
int muse_unmap(uint32_t dir) {
	...
	if(socketCliente != -1) {
		...
		if(retorno != -1) {
				...
				loggearWarning("No se ha podido eliminar el archivo de mappeo");
			}
	}
...
void muse_close() {
	...
	if(socketCliente != -1) {
		...
		if(retorno != -1) {
				...
				loggearWarning("No se ha podido cerrar la biblioteca, servidor MUSE caido");
			}
	}
